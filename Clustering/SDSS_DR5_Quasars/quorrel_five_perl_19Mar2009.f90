...
!!
!! wp(sigma) can be used and inverted/interpolated to work out
!! the real-space correlation, xi(r).
!!
!! 2 (two) input files are needed, the DATA and a random catalogue file, 
!! in the format of: 
!!     Object R.A., Object Dec, Object redshift, Object comoving distance
!! where the comoving distance is in h^-1 Mpc, determined (for our purposes)
!! with the currently preferred flat, "Lambda Cold Dark Matter" (LCDM) 
!! cosmology where Omega_b = 0.042, Omega_M = 0.237, and Omega_Lam = 0.763 
!! (e.g. Sanchez et al. 2006; Spergel et al. 2007).
!!
!!
!! NOTE THIS CODE IS FAIRLY CLUNKY, as in there are (LOTS of) variables etc. 
!! that are remmants from previous incarnations and now may well do nothing. 
!!
!! The code is compiled as e.g.:
!! > g95 quorrel_five_perl.f90 -o quorrel_five_perl.x
!! 
!! i.e. the subroutines tabulate_dist and search_dist are NOT required
!! Though, of course, more complier options can be used with totally
!! unknown effects. 
!!
!! And then to run:
!! > ./quorrel_five_perl.x 
!! 
!!
!! As a rough guide, running this code on the UNIFORM SDSS DR5 data set
!! with redshift cuts, 0.30<z<2.20, (30,239 quasasr) and the 
!! associated randoms took O(3days) on a effectively a single 
!! 2.66 GHz Dual-Core Intel `Xeon' processor (of a quad-core
!! MacPro). However, running the ~3000 quasar strong redshift 
!! subsamples takes O(30 mins).  
!! However, some proper benchmarking ought to be done here! 
!!
!!
!! Any use of this code should cite 
!! Ross et al. (2009, ApJ accepted, 0903.3230v1) 
!! and any improvements or problems should be emailed to: 
!! npr@astro.psu.edu
!!
!! Version history:
!!        18th January 2008   v1.0.0    Nic Ross   npr@astro.psu.edu
!!
!!
!!------------------------------------------------------------------------------
!!------------------------------------------------------------------------------
...
!!
!! This is fine since if one wants to include fibre collisions, that's 
!! already in the data e.g. ../data/DR5QSO_uni_data_wFibre.dat
!!
!! Note w_theta_2SLAQ_Sam8_v3.dat is supplied, but any explict use of this
!! should reference Ross et al. (2007, MNRAS, 381, 573).
!!
...
!!  I / P    F I L E S 
!!
!! The input files given below are for the UNIFORM DR5 Quasar sample. 
!!
!!
...
!! O / P    F I L E S
!! 
!! These filenames are malluable. 
!!
!! "k_output"      is the xi(s) output, spaced in log (10 bins/decade).
!! "k_output_log"  is the xi(s) output, linearly spaced.
!! "k2d_output"    is the "2D", xi(rp,pi) output file. 
!! "K_wp_output"   is the wp(rp) output file, having summed xi(rp,pi) over pi. 
!! 
!! "z_values"            gives a redshift distribution of the data 
!! "ra_dec_z_DR5Q_used"  gives the RAs,Decs and z that are being used for data
!! "ra_dec_z_randoms"   gives the RAs,Decs and z that are being used for randoms
!!
!! e.g. the *0pnt92z1pnt13* files were used for the 0.92<z<1.13 
!! redshift sub-sample.
!!
...
!! Setting up bin widths etc. for doing the N(z)'s. 
zbin_width = 0.05
...
ra_max = 360.0000
!!
!! Right Ascension limits for the jackknife calculations:
!!
...
dec_max = 70.000
!!
!! Declination limits for the jackknife calculations.
!! 
...
!!
!! Redshift boundaries. 
!! Matches Croom et al. (2005, MNRAS, 356, 415).
!!
...
!      pi_para_two = r_c(j)
      ...
      if (s .ne. 0 .and. sigma .ne. 0 .and. pi_para .ne. 0) then
         ...
         !! This have always worked!!!!
         !k_sigma   = int((log10(sigma)*10) + 12) 
         !k_pi_para = int((log10(pi_para)*10) + 12)
         !! If you want to try 10 bins/decade. 
         !! DD counts and S/N already low though!
         
         !k_sigma   = int(sigma)   +1 
         !k_pi_para = int(pi_para) +1 
         !!USE FOR xi(sigma,pi) "Petal plots" 
         ...
         !! WITHOUT w_theta correction!! 
         !! k_lin 56 => cut-off at s=275
...
!! 3RD LOOP 3RD LOOP 3RD LOOP  3RD LOOP 3RD LOOP 3RD LOOP  3RD LOOP 3RD LOOP
!!
!! Now working out the RR pairs. Since R = >20 D and this is an N^2 
!! exercise, the majority of the time program spends is in this 
!! double implied do loop. 
!! 
do i=1,N_rnd-1
   do l=1,10
...
!!
!! Calculating xi(s) and writing it out....
!!
if( choice_two .ne. 8 .and. choice_two .ne. 9) then
nRD_ratio = ( real(N_rnd) / real(N_LRG_sample))
...
!!
!! Calculating xi(s) LINEAR and writing it out....
!!
do k_lin=1,56
...
!!
!! Calculating xi(rp,pi) and wp(rp) and writing it out....
!!
...
!!
!! Calculating the N(z) and writing it out....
!!
write(*,*)
write(*,*)
write(*,*) 'almost there..'
