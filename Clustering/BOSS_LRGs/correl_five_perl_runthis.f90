...
real*8:: small_DD, small_DR, small_RR
...
elseif(jack .eq. 37) then 
   open(1,file='3yr_obj_v4_Sample8_nota01ao2s01_xyz.dat')
   open(2,file='3yr_obj_v4_Sample8_nota01ao2s01_xyz_randoms.cat')
   open(9,file='k_output_temp.dat')
   open(18,file='k2d_output_temp.dat')
   open(27,file='Kcap_output_temp.dat')
end if
...
do i=1, N_LRG_sample-1
   ...
   do j=i+1,N_LRG_sample
      ...
      if (s .ne. 0 .and. sigma .ne. 0 .and. pi_para .ne. 0) then
         ...
         if (k .le. 11)   write(*,*) 'k', k, 'theta', theta, (int((log10(theta)*5.0)+22-0.5)), theta_cor 
         ...
         if (k .le. 56 .and.k .gt. 0) then
            ...
            if(theta .gt. 0.1) then
               ...
               !! Vargas-Magana et al., 2013, A&A 554, A131 
               bin_DD(k) = bin_DD(k) + 1
            else
...
do i=1,N_rnd-1
   ...
   !do j=75000,125000 
      ...
      !if (s .ne. 0) then
         ...
         !! Using Vargas-Magana et al., 2013, A&A 554, A131... 
         if (k .le. 56 .and. k .gt. 0)   bin_RR(k) = bin_RR(k) + 1   
...
do k=1,56
   if (bin_DR(k) .ne. 0 .and. bin_DD(k) .ne. 0) then
      ...
      if (choice_three .eq. 'y' .and. bin_RR(k) .ne.  0) then 
         ...
         !! Vargas-Magana et al., 2013, A&A 554, A131 
         small_DD = bin_DD(k) / ((N_LRG_sample * (N_LRG_sample - 1.)/2.))
         small_RR = bin_RR(k) / ((N_rnd        * (N_rnd        - 1.)/2.))
         small_DR = bin_DR(k) / (N_LRG_sample  * N_rnd)

         xi_LS(k)  = (small_DD - (2.*small_DR) + small_RR ) / small_RR
         xi_Ham(k) = ((small_DD * small_RR) / (small_DR**2)) - 1.0
