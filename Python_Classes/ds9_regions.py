...
#Put regions around any of the flagged images (2,4,6...)
	def flags(self):
		hdu=self.datafile[1].data
		file=open('Combined_ch1ch2_regions.reg','a') #Append to the file created above
		for i in range(len(hdu.X_IMAGE_1)): #Iterates over the x,y coordinates from Sextractor:
...
#To use put this file in the working directory, import it: from ds9_regions import * 
#Type r=Region('File.fits'), r.circle(), r.flags() 
#Can change the name of the Output table in lines 10 and 24, but the input file name is written in the function call
#Also, the hdu.NAME lines need to be changed to hdu.COL_NAME depending on the table being used
